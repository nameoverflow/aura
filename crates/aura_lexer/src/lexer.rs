...
mod tests {
    ...
    fn test_keywords() {
        ...
        assert_eq!(lex_no_eof("type concept instance"), vec![Type, Concept, Instance]);
        assert_eq!(lex_no_eof("match if else"), vec![Match, If, Else]);
        assert_eq!(lex_no_eof("for while in"), vec![For, While, In]);
        assert_eq!(lex_no_eof("return break continue"), vec![Return, Break, Continue]);
        assert_eq!(lex_no_eof("pub use module"), vec![Pub, Use, Module]);
        assert_eq!(lex_no_eof("async parallel race timeout yield"), vec![Async, Parallel, Race, Timeout, Yield]);
        assert_eq!(lex_no_eof("where forall requires ensures"), vec![Where, Forall, Requires, Ensures]);
    ...
    fn test_identifiers() {
        ...
        assert_eq!(lex_no_eof("user_service"), vec![Ident("user_service".into())]);
    ...
    fn test_string_literal() {
        ...
        assert_eq!(lex_no_eof(r#""hello\nworld""#), vec![StringLit("hello\nworld".into())]);
    ...
    fn test_string_escaped_braces() {
        assert_eq!(lex_no_eof(r#""Use {{expr}}""#), vec![StringLit("Use {expr}".into())]);
    }

    #[test]
    fn test_operators() {
        assert_eq!(lex_no_eof("+ - * / %"), vec![Plus, Minus, Star, Slash, Percent]);
        assert_eq!(lex_no_eof("== != < > <= >="), vec![Eq, NotEq, Lt, Gt, LtEq, GtEq]);
        assert_eq!(lex_no_eof("= |> -> ?"), vec![Assign, Pipeline, Arrow, Question]);
    ...
    #[test]
    fn test_delimiters() {
        assert_eq!(lex_no_eof("( ) { } [ ]"), vec![LParen, RParen, LBrace, RBrace, LBracket, RBracket]);
        assert_eq!(lex_no_eof(", : :: . ;"), vec![Comma, Colon, ColonColon, Dot, Semicolon]);
    ...
    fn test_function_def() {
        ...
        assert_eq!(tokens, vec![
            Def, Ident("add".into()), LParen,
            Ident("a".into()), Colon, UpperIdent("Int".into()), Comma,
            Ident("b".into()), Colon, UpperIdent("Int".into()),
            RParen, Arrow, UpperIdent("Int".into()), Assign,
    ...
    fn test_type_def() {
        ...
        assert_eq!(tokens, vec![
            Type, UpperIdent("Status".into()), Assign,
            UpperIdent("Pending".into()), Pipe,
            UpperIdent("Processing".into()), Pipe,
            UpperIdent("Complete".into()), Pipe,
            UpperIdent("Failed".into()), UpperIdent("String".into()),
        ]);
    }
    ...
    fn test_use_import() {
        ...
        assert_eq!(tokens, vec![
            Use, Ident("std".into()), ColonColon,
            Ident("collections".into()), ColonColon,
    ...
    fn test_complex_expression() {
        ...
        assert_eq!(tokens, vec![
            Ident("x".into()), Pipeline,
            Ident("map".into()), LParen,
            LParen, Ident("y".into()), RParen, Arrow,
            Ident("y".into()), Star, IntLit(2),
            RParen,
        ]);
    }
    ...
    fn test_range_vs_dot() {
        ...
        assert_eq!(lex_no_eof("a.b"), vec![Ident("a".into()), Dot, Ident("b".into())]);
    }
    ...
    fn test_match_expression() {
        ...
        assert_eq!(tokens, vec![
            Match, Ident("status".into()), LBrace,
            UpperIdent("Pending".into()), FatArrow, IntLit(1), Comma,
            UpperIdent("Complete".into()), FatArrow, IntLit(2), Comma,
            Underscore, FatArrow, IntLit(0),
            RBrace,
        ]);
    }
